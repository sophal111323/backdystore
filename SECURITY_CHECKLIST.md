...
**What to test:**
- [ ] Visit `/admin/dystore`. Enter a wrong password â†’ should get a generic
  "Invalid email or password" error (no hint about whether the email exists).
- [ ] Enter wrong password **twice** â†’ account should be permanently locked.
  Verify in DB: `AdminAuthLock` table has `forever=true` for the identifier.
- [ ] Enter wrong password **once** â†’ account should be locked for 5 minutes.
...
**What to test:**
- [ ] Submit an invalid 6-digit code â†’ should get an error + lock warning.
- [ ] Submit a **valid** TOTP code â†’ should be fully logged in.
- [ ] Wait 30 seconds, get a new code â†’ should still work (1-step clock window).
- [ ] Submit a code **twice** (replay) â†’ second submission should fail because
...
- [ ] Send a POST to `/api/payment/webhook/khpay` **without** the
  `X-Webhook-Signature` header â†’ should return HTTP 401.
- [ ] Send with a **wrong** signature â†’ should return HTTP 401.
- [ ] Send the **same** `transaction_id` twice â†’ second request returns
  `{ ok: true, skipped: true, reason: "replay" }` and order is NOT updated again.
- [ ] Send with a mismatched `amount` â†’ order status set to FAILED, returns HTTP 400.
...
For each:
- [ ] Send requests up to the limit â†’ all succeed.
- [ ] Send one more â†’ HTTP 429 with `{ "error": "Too many requests. Please try again later." }`.
...
**What to test:**
- [ ] Upload a valid PNG â†’ succeeds, returns Cloudinary URL.
- [ ] Upload a valid JPG â†’ succeeds.
- [ ] Upload a valid WEBP â†’ succeeds.
- [ ] Upload an **SVG** â†’ should return HTTP 415 "Unsupported file type".
- [ ] Upload a **GIF** â†’ should return HTTP 415.
- [ ] Upload a **PDF** â†’ should return HTTP 415.
- [ ] Upload a file with a PNG extension but **wrong magic bytes** (e.g., rename a
  .exe to .png) â†’ should return HTTP 415 "File content does not match".
- [ ] Upload without being logged in â†’ should return HTTP 401.
- [ ] Upload a file > 5 MB â†’ should return HTTP 413.
...
- [ ] `ADMIN_JWT_SECRET` is at least 32 random characters (not "put-a-long-randomâ€¦").
...
[[ "$NODE_ENV" == "production" ]] && \
  ...
  echo "âœ… Env looks safe" || echo "âŒ Check your env vars"
```
...
- [ ] Confirm `telegramBotToken` is shown as `"â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢xxxx"` (last 4 chars only),
  **not** the full token.
- [ ] Update settings with a new Telegram bot token via the admin UI â†’ should save correctly.
- [ ] Refresh page â†’ masked value still shown, not the full token.
...
- [ ] Wait 8+ hours (or manually expire the cookie) â†’ should redirect to login.
- [ ] Log out â†’ `admin_token` and `admin_2fa_pending` cookies are both cleared.
